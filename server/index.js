...
const i_env = {
   ...
   server: {
      host: process.env.WXCHAT_HOST || '127.0.0.1',
      port: parseInt(process.env.WXCHAT_PORT || '8080'),
      staticDir: process.env.WXCHAT_STATIC_DIR?i_path.resolve(process.env.WXCHAT_STATIC_DIR):null,
      httpsCADir: process.env.WXCHAT_HTTPS_CA_DIR?i_path.resolve(process.env.WXCHAT_HTTPS_CA_DIR):null,
   },
};
...
server.listen(i_env.server.port, i_env.server.host, () => {
   console.log(`wxCHAT SERVER is listening at ${i_env.server.host}:${i_env.server.port}`);
});
